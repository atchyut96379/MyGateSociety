  ...
  return (
    <>
      ...
      {paymentConfig?.enabled && paymentConfig.mode === "test" && (
        <div className="card" style={{ borderColor: "#e6a817", background: "#fff9eb" }}>
          <p style={{ margin: 0 }}>
            <strong>Test payment mode.</strong> No real money is charged. Switch to live Razorpay keys on Azure when KYC is complete.
          </p>
        </div>
      )}

      {paymentConfig?.enabled && paymentConfig.mode === "live" && (
        <div className="card" style={{ borderColor: "#0d6e4f", background: "#f0faf6" }}>
          <p style={{ margin: 0 }}>
            <strong>Secure online payment</strong> — UPI, cards, and net banking via Razorpay.
          </p>
        </div>
      )}

      {paymentConfig && !paymentConfig.enabled && pending.length > 0 && (
        <p className="muted">Online payment is not configured. Contact the society office to pay.</p>
      )}
