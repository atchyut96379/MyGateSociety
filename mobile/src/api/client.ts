...
async function request<T>(
  ...
  try {
    ...
    if (err instanceof Error && err.name === "AbortError") {
      throw new ApiError(408, "Server is not responding. Check your internet connection.");
    }
...
export const api = {
  ...
  loginConfig() {
    return request<{ bootstrap_mode: boolean; bootstrap_login_id: string }>("/auth/login-config");
  },
  ...
  updateProfile(token: string, body: Record<string, unknown>) {
    return request<User>("/auth/profile", { method: "PATCH", body: JSON.stringify(body) }, token);
  },
  ...
  createUser(token: string, body: Record<string, unknown>) {
    return request<CreateUserResponse>("/users", { method: "POST", body: JSON.stringify(body) }, token);
  },
  ...
  createVisitor(token: string, body: Record<string, unknown>) {
    return request<Visitor>("/visitors", { method: "POST", body: JSON.stringify(body) }, token);
  },
  ...
  createDelivery(token: string, body: Record<string, unknown>) {
    return request<Delivery>("/deliveries", { method: "POST", body: JSON.stringify(body) }, token);
  },
  ...
  createNotice(token: string, body: { title: string; body: string; pinned?: boolean }) {
    return request<Notice>("/notices", { method: "POST", body: JSON.stringify(body) }, token);
  },
  ...
  staffCheckOut(token: string, staffId: string) {
    return request<{ ok: boolean; message: string }>(`/staff/${staffId}/check-out`, { method: "POST" }, token);
  },
  ...
  vehicles(token: string, last4?: string) {
    const q = last4 ? `?last4=${encodeURIComponent(last4)}` : "";
  ...
  updateComplaint(token: string, id: string, body: Record<string, unknown>) {
    return request<Complaint>(`/complaints/${id}`, { method: "PATCH", body: JSON.stringify(body) }, token);
  },
  ...
  raiseSos(token: string, message?: string) {
    return request<SosAlert>("/sos", { method: "POST", body: JSON.stringify({ message }) }, token);
  },
  ...
  generateBills(token: string, month?: string) {
    return request<Bill[]>("/bills/generate", { method: "POST", body: JSON.stringify({ month }) }, token);
  },

  payBill(token: string, id: string, method = "CASH", reference?: string) {
